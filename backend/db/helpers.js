...
export async function createObjectTable() {
    ...
    try{
        ...
        return await neonConnection.query(
            ...
            `CREATE TABLE IF NOT EXISTS englishDefinitions (
...
export async function dropObjectTable() {
    ...
    try{
        ...
        return await neonConnection.query("DROP TABLE IF EXISTS englishDefinitions;");
        ...
        console.log("operation 'DT' complete");
    }
};
...
export async function populateObjectTable() {
  ...
  const objects =   [
    ...
    try{
        ...
        return await neonConnection.query(`
            INSERT INTO englishDefinitions (title, definition, example, links, week) 
            (SELECT title, definition, example, links, week 
            FROM json_populate_recordset(NULL::englishDefinitions, $1::JSON));`
        ...
        console.log("operation 'PT' complete");
    }
...
export async function resetObjectTable() {
    ...
    try{
        ...
        console.log("operation 'RT' complete");
    }
...
export async function OLDcreateObjectTable() {
...
export async function OLDdropObjectTable() {
  return await pool.query("DROP TABLE IF EXISTS englishDefinitions;");
}

export async function OLDpopulateObjectTable() {
  const objects =   [
    {
        "title": "Array",
        "definition": "An array can hold many values under a single name, and you can access the values by referring to an index number.",
...
export async function OLDresetObjectTable() {
  return [
    await dropObjectTable(),
    await createObjectTable(),
    await populateObjectTable(),
